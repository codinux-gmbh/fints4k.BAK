 ...
 */
abstract class KontoumsaetzeZeitraumBase(
    segmentId: CustomerSegmentId,
    segmentVersion: Int,
    segmentNumber: Int,
    account: Datenelementgruppe,
    parameter: GetTransactionsParameter,
    supportedCamtMessages: List<String>? = null
)
    : Segment(listOf(
        Segmentkopf(segmentId, segmentVersion, segmentNumber),
        account,
        if (supportedCamtMessages == null) DoNotPrintDatenelement() else object : Datenelementgruppe(supportedCamtMessages.map { AlphanumerischesDatenelement(it, Existenzstatus.Mandatory) }, Existenzstatus.Mandatory) { },
